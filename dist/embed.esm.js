/* fontfyi-embed v1.0.0 | MIT | https://widget.fontfyi.com */
...
function familyToGoogleFontsParam(family) {
  return family.replace(/\s+/g, "+");
}
var DEFAULT_SPECIMEN = "The quick brown fox jumps over the lazy dog";
function initFontPreviewTool(el, config) {
  var _a;
  const dataset = el.dataset;
  const family = (_a = dataset.family) != null ? _a : "";
  const shadow = createShadow(el, config);
  const container = createWidgetRoot(shadow, el, "creativefyi-font-preview-tool");
  if (!family) {
    renderError(container, "Missing data-family attribute.", config);
    return;
  }
  renderLoading(container);
  const fontLink = document.createElement("link");
  fontLink.rel = "stylesheet";
  fontLink.href = `https://fonts.googleapis.com/css2?family=${familyToGoogleFontsParam(family)}:wght@400;700&display=swap`;
  shadow.insertBefore(fontLink, shadow.firstChild);
  const style = shadow.querySelector("style");
  if (style) {
    style.textContent += `
      .creativefyi-fp-header {
        padding: 14px 16px 10px;
        background: var(--surface);
        border-bottom: 1px solid var(--border);
      }
      .creativefyi-fp-family-name {
      ...
      .creativefyi-fp-specimen-wrap {
        padding: 16px;
        border-bottom: 1px solid var(--border);
      }
      .creativefyi-fp-specimen {
      ...
      .creativefyi-fp-info-item strong { color: var(--text); }
      .creativefyi-fp-actions { padding: 10px 16px; }
      .creativefyi-fp-link {
        display: inline-flex; align-items: center; gap: 4px;
        color: var(--link); text-decoration: none; font-size: 13px; font-weight: 500;
      }
      .creativefyi-fp-link:hover { text-decoration: underline; }
      .creativefyi-fp-link svg { width: 13px; height: 13px; }
    `;
  }
  const slug = familyToSlug(family);
  fetchAPI(config.apiBase, `fonts/${slug}/`, {}).then((data) => {
    var _a2, _b, _c, _d, _e, _f;
    const displayName = (_b = (_a2 = data.family) != null ? _a2 : data.name) != null ? _b : family;
    const category = (_c = data.category) != null ? _c : "Sans-serif";
    const subcategory = (_d = data.subcategory) != null ? _d : "";
    const designer = (_e = data.designer) != null ? _e : "";
    const rank = data.popularity_rank;
    const isVariable = (_f = data.is_variable) != null ? _f : false;
    const fontUrl = `https://${config.domain}/fonts/${slug}/`;
    const metaBadges = [
      `<span class="creativefyi-badge">${escapeHTML7(category)}</span>`,
      subcategory ? `<span class="creativefyi-badge">${escapeHTML7(subcategory)}</span>` : "",
      isVariable ? `<span class="creativefyi-badge">Variable</span>` : ""
    ].filter(Boolean).join("");
    container.innerHTML = `
        <div class="creativefyi-fp-header">
          <p class="creativefyi-fp-family-name" style="font-family:'${escapeHTML7(family)}',sans-serif">${escapeHTML7(displayName)}</p>
          <div class="creativefyi-fp-meta">${metaBadges}</div>
        </div>
        <div class="creativefyi-fp-specimen-wrap">
          <textarea
            class="creativefyi-fp-specimen"
            style="font-family:'${escapeHTML7(family)}',system-ui,sans-serif"
            rows="3"
            spellcheck="false"
          >${escapeHTML7(DEFAULT_SPECIMEN)}</textarea>
        </div>
        <div class="creativefyi-fp-info">
          ${designer ? `<span class="creativefyi-fp-info-item">Designer: <strong>${escapeHTML7(designer)}</strong></span>` : ""}
          ${rank != null ? `<span class="creativefyi-fp-info-item">Popularity rank: <strong>#${rank}</strong></span>` : ""}
        </div>
        <div class="creativefyi-fp-actions">
          <a class="creativefyi-fp-link" href="${fontUrl}" target="_blank" rel="noopener">
            View ${escapeHTML7(displayName)} ${externalLinkIcon}
          </a>
        </div>
        ${poweredByHTML(config)}
      `;
    ...
    container.innerHTML = `
        <div class="creativefyi-fp-header">
          <p class="creativefyi-fp-family-name" style="font-family:'${escapeHTML7(family)}',sans-serif">${escapeHTML7(family)}</p>
        </div>
        <div class="creativefyi-fp-specimen-wrap">
          <textarea
            class="creativefyi-fp-specimen"
...
// src/tools/css-stack.ts
function escapeHTML8(str) {
  return str.replace(/&/g, "&amp;").replace(/</g, "&lt;").replace(/>/g, "&gt;").replace(/"/g, "&quot;").replace(/'/g, "&#39;");
}
function familyToSlug2(family) {
  return family.toLowerCase().replace(/\s+/g, "-");
}
var FALLBACK_CHAINS = {
  "sans-serif": "system-ui, -apple-system, 'Segoe UI', Roboto, sans-serif",
  "serif": "Georgia, 'Times New Roman', Times, serif",
  "monospace": "'Fira Code', 'Cascadia Code', Consolas, 'Courier New', monospace",
  "display": "Impact, 'Arial Black', cursive",
  "handwriting": "cursive"
};
function buildDeclaration(family, category) {
  var _a;
  const fallback = (_a = FALLBACK_CHAINS[category]) != null ? _a : FALLBACK_CHAINS["sans-serif"];
  const quoted = family.includes(" ") ? `'${family}'` : family;
  return `font-family: ${quoted}, ${fallback};`;
}
...
function initCssStackTool(el, config) {
  var _a;
  const dataset = el.dataset;
  const initialFamily = (_a = dataset.family) != null ? _a : "Inter";
  const shadow = createShadow(el, config);
  const container = createWidgetRoot(shadow, el, "creativefyi-css-stack-tool");
  const style = shadow.querySelector("style");
  if (style) {
    style.textContent += `
      .creativefyi-css-body { padding: 12px; }
      .creativefyi-css-input-row {
        display: flex; gap: 8px; margin-bottom: 12px;
      }
      .creativefyi-css-input {
        flex: 1; padding: 8px 10px;
        border: 1px solid var(--input-border); border-radius: 6px;
        background: var(--input-bg); color: var(--text);
        font-size: 13px; font-family: inherit; outline: none;
      }
      .creativefyi-css-input:focus {
        border-color: var(--input-focus);
        box-shadow: 0 0 0 2px color-mix(in srgb, var(--input-focus) 20%, transparent);
      }
      .creativefyi-css-cat-select {
        padding: 8px 10px;
        border: 1px solid var(--input-border); border-radius: 6px;
        background: var(--input-bg); color: var(--text);
        font-size: 12px; font-family: inherit; outline: none; cursor: pointer;
      }
      .creativefyi-css-output-wrap {
        position: relative; background: var(--surface);
        border: 1px solid var(--border); border-radius: 6px;
        padding: 12px 40px 12px 12px;
        font-family: 'Fira Code', 'Cascadia Code', Consolas, monospace;
        font-size: 12px; color: var(--text); line-height: 1.7; word-break: break-word;
        min-height: 44px;
      }
      .creativefyi-css-copy-btn {
        position: absolute; top: 8px; right: 8px;
        background: var(--copy-bg); border: none; border-radius: 5px;
      ...
      .creativefyi-css-copy-btn:hover { background: var(--copy-hover); }
      .creativefyi-css-copy-btn svg { width: 11px; height: 11px; }
      .creativefyi-css-hint {
        margin-top: 8px; font-size: 11px; color: var(--muted);
      }
    `;
  }
  container.innerHTML = `
    <div class="creativefyi-css-body">
      <div class="creativefyi-css-input-row">
        <input class="creativefyi-css-input" type="text" value="${escapeHTML8(initialFamily)}" placeholder="Font family name\u2026" spellcheck="false">
        <select class="creativefyi-css-cat-select" id="css-cat-select">
      ...
      <div class="creativefyi-css-output-wrap">
        <code id="css-output"></code>
        <button class="creativefyi-css-copy-btn" id="css-copy-btn">${copyIcon} Copy</button>
      </div>
      <p class="creativefyi-css-hint">Edit the font name or select category to update the declaration.</p>
    </div>
    ${poweredByHTML(config)}
  `;
  const input = container.querySelector(".creativefyi-css-input");
  const catSelect = container.querySelector("#css-cat-select");
  const output = container.querySelector("#css-output");
  const copyBtn = container.querySelector("#css-copy-btn");
  let currentDeclaration = "";
  function render(family, cat) {
    currentDeclaration = buildDeclaration(family, cat);
    output.textContent = currentDeclaration;
  }
  function update() {
    render(input.value || "Inter", catSelect.value);
  }
  copyBtn.addEventListener("click", () => copyText(copyBtn, currentDeclaration));
  input.addEventListener("input", update);
  catSelect.addEventListener("change", update);
  render(initialFamily, "sans-serif");
  if (initialFamily) {
    fetchAPI(config.apiBase, `fonts/${familyToSlug2(initialFamily)}/`, {}).then((data) => {
      if (data.category) {
        const cat = normalizeCategory(data.category);
        catSelect.value = cat;
        render(initialFamily, cat);
      }
    }).catch(() => {
    });
  }
...
// src/inline/font-name.ts
var MONO_KEYWORDS = ["mono", "code", "console", "terminal", "courier"];
var SERIF_KEYWORDS = [
  "serif",
  "playfair",
  "merriweather",
...
function toGoogleFontsParam(family) {
  return family.trim().replace(/\s+/g, "+");
}
function initFontNameInline(el, config) {
  const family = el.dataset.family || "Inter";
  const category = el.dataset.category || inferCategory(family);
  const shadow = el.attachShadow({ mode: "open" });
  const googleFontsParam = toGoogleFontsParam(family);
  const googleFontsUrl = `https://fonts.googleapis.com/css2?family=${googleFontsParam}:wght@400;700&display=swap`;
  const style = document.createElement("style");
  style.textContent = `
  ...
  shadow.appendChild(categorySpan);
  el.addEventListener("click", () => {
    var _a;
    (_a = navigator.clipboard) == null ? void 0 : _a.writeText(family).catch(() => {
      const ta = document.createElement("textarea");
      ta.value = family;
      ta.style.position = "fixed";
      ta.style.opacity = "0";
  ...
  el.title = `${family} (${category}) \u2014 Click to copy \u2014 ${config.name}`;
}

// src/_entry_fontfyi.ts
function initWidget(el, type, config) {
  ...
  switch (type) {
    ...
    case "tooltip":
      ...
      break;
    case "preview":
      initFontPreviewTool(el, config);
      break;
    case "stack":
      initCssStackTool(el, config);
      break;
    case "font-name":
      initFontNameInline(el, config);
      break;
    default:
      break;
  }
...
(function bootstrap() {
  const config = '{"site":"fontfyi","name":"FontFYI","domain":"fontfyi.com","accent":"#8B5CF6","attribute":"data-fontfyi","apiBase":"https://fontfyi.com/api/v1/","searchPath":"/search/","entityName":"Fonts","entitySlug":"fonts"}';
...
function makeWidgetElement(widgetType, initFn, domainAttrs) {
  ...
  return class extends HTMLElement {
    ...
    connectedCallback() {
      ...
      this._syncDataAttrs();
      initFn(this, '{"site":"fontfyi","name":"FontFYI","domain":"fontfyi.com","accent":"#8B5CF6","attribute":"data-fontfyi","apiBase":"https://fontfyi.com/api/v1/","searchPath":"/search/","entityName":"Fonts","entitySlug":"fonts"}');
    }
    ...
    _syncDataAttrs() {
      const attrKey = '{"site":"fontfyi","name":"FontFYI","domain":"fontfyi.com","accent":"#8B5CF6","attribute":"data-fontfyi","apiBase":"https://fontfyi.com/api/v1/","searchPath":"/search/","entityName":"Fonts","entitySlug":"fonts"}'.attribute.replace("data-", "");
...
(function registerElements() {
  ...
  const site = '{"site":"fontfyi","name":"FontFYI","domain":"fontfyi.com","accent":"#8B5CF6","attribute":"data-fontfyi","apiBase":"https://fontfyi.com/api/v1/","searchPath":"/search/","entityName":"Fonts","entitySlug":"fonts"}'.site;
  const defs = [
    ...
    [`${site}-preview`, initFontPreviewTool, ["slug", "value"]],
    [`${site}-stack`, initCssStackTool, ["slug", "value"]],
    [`${site}-font-name`, initFontNameInline, ["family", "label"]]
  ];
